...
export default function CreateSessionPage() {
  const { createNewSession, isLoading: contextIsLoading, error: contextError, clearSessionData } = useContext(SessionContext);
    ...
    if (reason === 'input' && newValue && newValue.length > 2) {
      ...
      try {
        ...
        try {
            ...
            setLocation(geocodeResponse.data.full_address || newValue);
            setAutocompleteOptions([]);
        } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!hostName || !location || !locationDetails) {
      setFormError('Host name and a geocoded location are required.');
      return;
    }
    setFormError('');
    setInviteUrlLocal('');
    ...
    try {
      ...
      console.log("Session created via context:", sessionData);
      setInviteUrlLocal(sessionData.invite_url);
      setCreatedSessionId(sessionData.session_id);
      setCreatedPlayerId(sessionData.player_id);
      ...
      >
        <Container maxWidth="sm">
          <Paper elevation={6} sx={{ p: 4, borderRadius: 2, textAlign: 'center' }}>
            ...
            <TextField
              fullWidth
              value={inviteUrlLocal}
              readOnly
              sx={{ mb: 2 }}
              onClick={(e) => e.target.select()}
            />
            <Box sx={{display: 'flex', justifyContent: 'center', gap: 1, flexWrap: 'wrap'}}>
                <Button variant="contained" onClick={() => navigator.clipboard.writeText(inviteUrlLocal)}>
                Copy Link
                </Button>
                {createdSessionId && createdPlayerId && (
                    <Button variant="contained" color="primary" onClick={handleGoToSession}>
                        Go to Waiting Room
                    </Button>
                )}
                <Button variant="outlined" onClick={() => { setInviteUrlLocal(''); navigate('/');}}>
                Back to Home
                </Button>
            </Box>
